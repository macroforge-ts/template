...
impl Codegen {
        ...
        match node {
            ...
            IrNode::Placeholder { kind, expr, .. } => {
                match kind {
                    ...
                    _ => Err(GenError::invalid_placeholder(
                        "pattern",
                        &format!("{:?}", kind),
                        &["Ident", "Expr"],
                    )),
            ...
            IrNode::ObjectPat {
                ...
                let props_code: Vec<TokenStream> = props
                    .iter()
                    .map(|prop| self.generate_object_pat_prop(prop))
                    .collect::<GenResult<Vec<_>>>()?;
                let type_ann_code = match type_ann.as_ref() {
                    Some(t) => {
                        let tc = self.generate_type_ann(t)?;
    ...
    pub(super) fn generate_pats(&self, nodes: &[IrNode]) -> GenResult<TokenStream> {
        let pats_code: Vec<TokenStream> = nodes
            .iter()
